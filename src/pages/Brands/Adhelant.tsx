...
const Adhelant: React.FC = () => {
  const handleWhatsAppClick = (productName: string) => {
    const message = `I want to buy the ${productName} from Adhelant products`;
      ...
      <div className="text-center mb-16">
        <div className="flex items-center justify-center gap-4 mb-2">
          ...
          <h2 className="text-2xl md:text-3xl font-black text-center tracking-tighter uppercase italic text-black/90">
            Adhelant <span className="text-[#6B5E18] ml-2">Products</span>
          </h2>
          <div className="h-[1px] bg-gray-200 flex-grow max-w-[100px]"></div>
        </div>
        <p className="text-gray-500 text-base max-w-2xl mx-auto leading-relaxed">
          Discover Adhelant's premium epoxy adhesive systems for construction and industrial applications. 
          High-performance bonding solutions engineered for structural integrity and long-lasting durability.
        </p>
      </div>

      {/* Grid */}
      <div className="max-w-[1400px] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
        {AdhelantProducts.map((product) => (
